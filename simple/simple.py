...
class Layer:
    ...
    def __init__(self, n, k, device, root='./simple_configs'):
        ...
        # Read in circuit
        # vtree = Vtree.read('4C2.vtree')
        # manager = SddManager(vtree)
        # alpha = io.sdd_read('4C2.sdd', manager)
        ...
        parents = torch.empty((id, max_parents, 2), dtype=torch.long, device=device).fill_(id)
        for k,v in parents_dict.items():
            parents[k] = torch.tensor(v + [[id, 0]]*(max_parents - len(v)), dtype=torch.long, device=device)
        ...
        for level in levels_nodes:
            levels.append(torch.tensor([l.id for l in level], dtype=torch.long, device=device))
        levels.reverse()
        self.levels = levels
        ...
        # true indices
        true_indices = torch.tensor([node.id for node in nodes if node.is_true()], dtype=torch.long, device=device)
        self.true_indices = true_indices

        # Literal indices
        literal_indices = torch.tensor([[node.id, node.literal] for node in nodes if node.is_literal()], dtype=torch.long, device=device)
        ...
        idx2primesub = torch.zeros((id, max_elements, 2), dtype=torch.long, device=device)
        for node in nodes:
            if node.is_decomposition():
                tmp = torch.tensor([[p.id, s.id] for p, s in node.elements], dtype=torch.long)
...
#     for i in range(10):
#         log_probs = torch.ones((batch_size, 4096), device='cuda', requires_grad=True).log()
#         log_probs[:, -1] = 0
#         start = time.time()
